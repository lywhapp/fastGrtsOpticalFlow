...
int fast_patch_dist<5, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  ...
  for (int dy = 0; dy < 5; dy++) {
    int *row2 = ((int *) b->line[by+dy])+2*bx;
    {
      unsigned int c1 = adata[0];
      unsigned int c2 = row2[0];
      int dr = (c1&255)-(c2&255);
      int dg = ((c1>>8)&255)-((c2>>8)&255);
      int db = ((c1>>16)&255)-((c2>>16)&255);
	  int dxc = (c1>>24)-(c2>>24);
     
	  unsigned int c3 = adata[1];
      unsigned int c4 = row2[1];
	  int dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  int dgy = (short)((c3>>16))-(short)((c4>>16));

      ans += dr*dr+dg*dg+db*db+dxc*dxc+dgx*dgx+dgy*dgy;
      if (ans > maxval) { return ans; }
    }
    {
      unsigned int c1 = adata[2];
	  ...
	  int dxc = (c1>>24)-(c2>>24);
	   ...
	   unsigned int c3 = adata[3];
      unsigned int c4 = row2[3];
	  int dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  int dgy = (short)((c3>>16))-(short)((c4>>16));
    ...
    }
    {
     ...
     int db = ((c1>>16)&255)-((c2>>16)&255);
      ...
      unsigned int c3 = adata[5];
      unsigned int c4 = row2[5];
	  int dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  int dgy = (short)((c3>>16))-(short)((c4>>16));
    ...
    }
    {
	  ...
	  int dxc = (c1>>24)-(c2>>24);
	   ...
	   unsigned int c3 = adata[7];
      unsigned int c4 = row2[7];
	  int dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  int dgy = (short)((c3>>16))-(short)((c4>>16));
    ...
    {
      unsigned int c1 = adata[8];
      unsigned int c2 = row2[8];
      int dr = (c1&255)-(c2&255);
      int dg = ((c1>>8)&255)-((c2>>8)&255);
      int db = ((c1>>16)&255)-((c2>>16)&255);
	  int dxc = (c1>>24)-(c2>>24);

	  unsigned int c3 = adata[9];
      unsigned int c4 = row2[9];
	  int dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  int dgy = (short)((c3>>16))-(short)((c4>>16));
...
template<>
int fast_patch_dist_v2<1, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (IS_MASK && bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int *row2 = ((int *) b->line[by])+bx;
  {
    unsigned int c1 = adata[0];
...
template<>
int fast_patch_dist_v2<2, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 2; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_dist_v2<3, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 3; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_dist_v2<4, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 4; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_dist_v2<5, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 5; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_dist_v2<6, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 6; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_dist_v2<7, 0>(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 7; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<1, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int *row2 = ((int *) b->line[by])+bx;
  {
    unsigned int c1 = adata[0];
...
template<>
int fast_patch_nobranch_v2<2, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 2; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<3, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 3; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<4, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 4; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<5, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 5; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<6, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 6; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
template<>
int fast_patch_nobranch_v2<7, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  int ans = 0;
  for (int dy = 0; dy < 7; dy++) {
    int *row2 = ((int *) b->line[by+dy])+bx;
...
int fast_patch_nobranch<5, 0>(int *adata, BITMAP *b, int bx, int by, Params *p) {
  ...
  for (int dy = 0; dy < 5; dy++) {
    int *row2 = ((int *) b->line[by+dy])+2*bx;
    {
	  ...
	  unsigned int c3 = adata[1];
      unsigned int c4 = row2[1];
	  short dgx = (short)(c3&0xffff)-(short)(c4&0xffff);
	  short dgy = (short)((c3>>16))-(short)((c4>>16));
