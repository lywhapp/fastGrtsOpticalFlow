...
BITMAP *convert_bitmap(IplImage *A) 
{
	if (A->nChannels!= 3) { mexErrMsgTxt("color channels != 3"); }
	// matlab, h * w * n: (y, x, i) -> y + x * h + i * w * h
	const int h = A->height;
	const int w = A->width;
	const int widestep=A->widthStep;
	{
		unsigned char *data =(uchar*)A->imageData;
...
BITMAP * CopyBitmap(BITMAP *a)
{
	BITMAP *Des = create_bitmap(a->w, a->h);
	//ans->data = new unsigned char[4*w*h]; 
	memcpy(Des->data,a->data,4*a->w*a->h);
	return Des;
}
