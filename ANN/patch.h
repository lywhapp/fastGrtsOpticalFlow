...
int fast_patch_dist_v2(int *adata, BITMAP *b, int bx, int by, int maxval, Params *p) {
  if (IS_WINDOW) {
    int ans = 0;
...
// !!!notice that "err", "xbest", "ybest" might be changed in this function
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
void attempt_n_v2(int &err, int &xbest, int &ybest, int *adata, BITMAP *b, int bx, int by, BITMAP *bmask, RegionMasks *region_masks, int src_mask, Params *p) {
  if ((bx != xbest || by != ybest) &&
      (unsigned) bx < (unsigned) (b->w-PATCH_W+1) &&
      (unsigned) by < (unsigned) (b->h-PATCH_W+1)) 
	{
    if (IS_MASK && region_masks && src_mask != ((int *) region_masks->bmp->line[by])[bx]) { return; }
    if (IS_MASK && bmask && ((int *) bmask->line[by])[bx]) { return; }
    int current = fast_patch_dist_v2<PATCH_W, IS_WINDOW>(adata, b, bx, by, err, p);
    if (current < err) {
      err = current;
      xbest = bx;
      ybest = by;
...
template<int TPATCH_W, int IS_WINDOW>
int fast_patch_nobranch_v2(int *adata, BITMAP *b, int bx, int by, Params *p) {
  //if (IS_MASK && bmask && ((int *) bmask->line[by])[bx]) { return INT_MAX; }
  if (IS_WINDOW) {
    int ans = 0;
    for (int dy = 0; dy < TPATCH_W; dy++) {
