...
void init_openmp(Params *p) {
  if (p->cores != last_cores&&p->cores>1) {
    last_cores = p->cores;
#if USE_OPENMP
  ...
  for (int y = box.ymin; y < box.ymax; y++) {
    ...
    for (int x = box.xmin; x < box.xmax; x++) {
      ...
      for (int dy = 0; dy < PATCH_W; dy++) { // copy a patch from a to adata
        int *drow = ((int *) a->line[y+dy])+2*x;
        int *adata_row = adata+(dy*PATCH_W*2);
        for (int dx = 0; dx < PATCH_W*2; dx++) {
...
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
BITMAP *init_dist_n_v2(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
  BITMAP *ans = create_bitmap(a->w, a->h);
  //clear_to_color(ans, INT_MAX);
  if (region_masks) { // XC?, why the second line?
    if (region_masks->bmp->w != a->w || region_masks->bmp->h != a->h) { fprintf(stderr, "region_masks (%dx%d) size != a (%dx%d) size\n", region_masks->bmp->w, region_masks->bmp->h, a->w, a->h); exit(1); }
...
#pragma omp parallel for schedule(static, 4)
  for (int y = box.ymin; y < box.ymax; y++) {
    ...
    for (int x = box.xmin; x < box.xmax; x++) {
      ...
      if (IS_MASK && bmask && ((int *) bmask->line[yp])[xp]) { row[x] = INT_MAX; continue; }
      row[x] = fast_patch_nobranch_v2<PATCH_W, IS_WINDOW>(adata, b, xp, yp, p);
      //if (x == 1 && y == 1) { printf("1, 1 => %d, %d (%d)\n", xp, yp, row[x]); }
    }
  }
  return ans;
...
template<int PATCH_W/*块的大小*/, int IS_MASK/*是否使用a图像的计算掩码*/, int IS_WINDOW/*是否使用约束*/>
void nn_n(Params *p, BITMAP *a, BITMAP *b,
          BITMAP *ann, BITMAP *annd,
          RegionMasks *amask/*输入掩码，包含一个包围框和对应的掩码图像*/, 
		  //-------------------------------------------以下为不常用参数------------------------------------------------
		  BITMAP *bmask,
          int level1/*未使用*/, int em_iter1/*未使用*/, RecomposeParams *rp1/*未使用*/, 
		  int offset_iter/*迭代次数偏移，可以控制是先奇数次还是偶数次*/,
		  int update_type1/*未使用*/, RegionMasks *region_masks/*分割局域Mask，如仅对同一分割标签的两个块进行匹配*/,
		  int tiles1/*未使用*/,BITMAP *ann_window/*有约束时，提供约束所需的ANN场*/, BITMAP *awinsize/*有约束时使用*/) 
{
//#define ignoreErr(err0,err) ((err0)-(err)<(p->do_randomSearch?0:100))
//#define funWt(d)		0/*(100*(1.0-exp(-(d)/200.0)))*/
//#define funWt2(d)  (200*(exp(-(d)/500.0)))
	//err0为与相邻像素保持相同光流场的误差
#define ignoreErr1(err0,err) ((err0)-(err)<(500))
//#define ignoreErr(err0,err,dis0,dis) ((err0)+funWt(dis0)<(err)+100+funWt(dis))
	//#define ignoreErr(err0,err) ((err0)<(err)+200*exp(-((err0)+(err))/20000.0))
  ...
  bool isSubFlow=p->SubPixelsFlow!=NULL;
  FlowType* ptrFlow=p->SubPixelsFlow+(ann->w*(PATCH_W/2)+(PATCH_W/2))*2;
  const int FlowWidthStep=ann->w*2;
  for (; nn_iter < p->nn_iters; nn_iter++) {
    ...
    int adata[PATCH_W*PATCH_W*2];
	//uint aColLine[2][PATCH_W],bColLine[2][PATCH_W],aRowLine[2][PATCH_W],bRowLine[2][PATCH_W];
    for (int y = ystart; y != yfinal; y += ychange) {
      ...
      for (int x = xstart; x != xfinal; x += xchange) {
        ...
        int xbest, ybest;
        if(!getnn(ann, x, y, xbest, ybest))
			continue;

        int err = annd_row[x];
        if (err == 0) { continue; }

		int xflo,yflo,needUpdate=0;		
        /* Propagate */
        if (p->do_propagate) {
          if (!IS_WINDOW) {
			  bool condPy=(unsigned) (y+dy) < (unsigned) (ann->h-PATCH_W);
            /* Propagate x */
            if ((unsigned) (x+dx) < (unsigned) (ann->w-PATCH_W)) {
              int xpp, ypp;
              if(getnn(ann, x+dx, y, xpp, ypp)) {
              xpp -= dx;
    ...
    //              int c4 = c4row[xi];
                ...
                for (int yi = 0; yi < PATCH_W; yi++) {
				  uint* ptrc1 = ((uint *) a->line[y+yi])+2*(x+xa);
                  uint* ptrc2 = ((uint *) b->line[ypp+yi])+2*(xpp+xa);
                  uint* ptrc3 = ((uint *) a->line[y+yi])+2*(x+xb+PATCH_W-1);
                  uint* ptrc4 = ((uint *) b->line[ypp+yi])+2*(xpp+xb+PATCH_W-1);
                  ...
                  int db12 = ((c1>>16)&255)-((c2>>16)&255);
				  int dx12 = (c1>>24)-(c2>>24);

                  int dr34 = (c3&255)-(c4&255);
                  int dg34 = ((c3>>8)&255)-((c4>>8)&255);
                  int db34 =((c3>>16)&255)-((c4>>16)&255);
				  int dx34 = (c3>>24)-(c4>>24);

				  uint c5 =ptrc1[1];
                  uint c6 = ptrc2[1];
                  uint c7 = ptrc3[1];
                  uint c8 = ptrc4[1];
				  int dgx12 = (short)(c5&0xffff)-(short)(c6&0xffff);
                  int dgy12 = (short)((c5>>16))-(short)((c6>>16));

                  int dgx34 = (short)(c7&0xffff)-(short)(c8&0xffff);
                  int dgy34 = (short)((c7>>16))-(short)((c8>>16));

                  partial +=  dr34*dr34+dg34*dg34+db34*db34+dx34*dx34+dgx34*dgx34+dgy34*dgy34
                             -dr12*dr12-dg12*dg12-db12*db12-dx12*dx12-dgx12*dgx12-dgy12*dgy12;
                }
                err0 += (dx < 0) ? partial: -partial; 
				//int dis=(xbest-x)*(xbest-x)+(ybest-y)*(ybest-y);
				//int dis0=(xpp-x)*(xpp-x)+(ypp-y)*(ypp-y);
				if (/*err0 < err*/ignoreErr1(err0,err)) {
                  err = err0;
                  xbest = xpp;
                  ybest = ypp;
            ...
            if ((unsigned) (y+dy) < (unsigned) (ann->h-PATCH_W)) {
              ...
              if ((xpp != xbest || ypp != ybest) &&
                ...
                if (dy > 0) { ya = 0; yb = dy; }
                int partial = 0;
				int *c1row = &((int *) a->line[y+ya])[2*x];
                int *c2row = &((int *) b->line[ypp+ya])[2*xpp];
                int *c3row = &((int *) a->line[y+yb+PATCH_W-1])[2*x];
                int *c4row = &((int *) b->line[ypp+yb+PATCH_W-1])[2*xpp];
                for (int xi = 0; xi < PATCH_W; xi++) {
                  uint c1 = *(c1row++);
                  uint c2 = *(c2row++);
                  uint c3 = *(c3row++);
        ...
        /* Random search */
		if (p->do_randomSearch){
			
			for (int dy0 = 0; dy0 < PATCH_W; dy0++) { // copy a patch from a
				int *drow = ((int *) a->line[y+dy0])+2*x;
				int *adata_row = adata+(dy0*PATCH_W*2);
				for (int dx0 = 0; dx0 < PATCH_W*2; dx0++) {
		...
		if(isSubFlow&&needUpdate)
		{
			int idxDst=2*x+y*FlowWidthStep;
			int idxSrc=2*xflo+yflo*FlowWidthStep;
			ptrFlow[idxDst]=ptrFlow[idxSrc];
			ptrFlow[idxDst+1]=ptrFlow[idxSrc+1];
		}
      }
    }
  }
...
template<int PATCH_W/*块的大小*/, int IS_MASK/*是否使用a图像的计算掩码*/, int IS_WINDOW/*是否使用约束*/>
void nn_n_v2(Params *p, BITMAP *a, BITMAP *b,
          BITMAP *ann, BITMAP *annd,
          RegionMasks *amask/*输入掩码，包含一个包围框和对应的掩码图像*/, 
		  //-------------------------------------------以下为不常用参数------------------------------------------------
		  BITMAP *bmask,
...
//#define funWt2(d)  (200*(exp(-(d)/500.0)))
	//err0为与相邻像素保持相同光流场的误差
#define ignoreErr2(err0,err) ((err0)-(err)<(100))
//#define ignoreErr(err0,err,dis0,dis) ((err0)+funWt(dis0)<(err)+100+funWt(dis))
	//#define ignoreErr(err0,err) ((err0)<(err)+200*exp(-((err0)+(err))/20000.0))
  //printf("in nn_n, masks are: %p %p %p, tiles=%d\n", amask, bmask, region_masks, tiles);
  ...
  for (; nn_iter < p->nn_iters; nn_iter++) {
          ...
          if (!IS_WINDOW) {
            ...
            if ((unsigned) (x+dx) < (unsigned) (ann->w-PATCH_W)) {
              ...
              if ((xpp != xbest || ypp != ybest) &&
				...
				{
					xa = 0; xb = dx; 
					memcpy(aRowLine[0],a->line[y+0]+4*x,PATCH_W*4);
					//memcpy(bRowLine[0],b->line[ypp+0]+xpp+xa,PATCH_W*4);
				}
				else if(condPy)
				{
					memcpy(aRowLine[0],a->line[y-1]+4*x,PATCH_W*4);
					//memcpy(bRowLine[0],b->line[ypp-1]+xpp+xa,PATCH_W*4);
				}
				 for (int yi = 0; yi < PATCH_W; yi++) 
				 {
					 aColLine[0][yi] = ((int *) a->line[y+yi])[x+xa];
					 aColLine[1][yi] = ((int *) a->line[y+yi])[x+xb+PATCH_W-1];

					 //bColLine[0][yi] = ((int *) b->line[ypp+yi])[xpp+xa];               
					 //bColLine[1][yi] = ((int *) b->line[ypp+yi])[xpp+xb+PATCH_W-1];
				 }
				 if(dx<0)
				 {
					 memcpy(aRowLine[1],a->line[y+PATCH_W-1]+4*x,PATCH_W*4);
					 //memcpy(bRowLine[1],b->line[ypp+PATCH_W-1]+xpp+xa,PATCH_W*4);
				 }
				 else if(condPy)
				 {
					  memcpy(aRowLine[1],a->line[y+PATCH_W]+4*x,PATCH_W*4);
					  //memcpy(bRowLine[1],b->line[ypp+PATCH_W]+xpp+xa,PATCH_W*4);
				 }
				  for (int yi = 0; yi < PATCH_W; yi++) 
				 {
					 //aColLine[0][yi] = ((int *) a->line[y+yi])[x+xa];
    ...
    //              int c4 = c4row[xi];
                ...
                for (int yi = 0; yi < PATCH_W; yi++) {
                  ...
                  int dr12 = (aColLine[0][yi]&255)-(bColLine[0][yi]&255);
                  int dg12 = ((aColLine[0][yi]>>8)&255)-((bColLine[0][yi]>>8)&255);
                  int db12 = ((aColLine[0][yi]>>16))-((bColLine[0][yi]>>16));

                  int dr34 = (aColLine[1][yi]&255)-(bColLine[1][yi]&255);
                  int dg34 = ((aColLine[1][yi]>>8)&255)-((bColLine[1][yi]>>8)&255);
                  int db34 =((aColLine[1][yi]>>16))-((bColLine[1][yi]>>16));
				...
				//int dis=(xbest-x)*(xbest-x)+(ybest-y)*(ybest-y);
				//int dis0=(xpp-x)*(xpp-x)+(ypp-y)*(ypp-y);
				if (/*err0 < err*/ignoreErr2(err0,err)) {
                  err = err0;
                  xbest = xpp;
                  ybest = ypp;
            ...
            /* Propagate y */
            if (condPy/*(unsigned) (y+dy) < (unsigned) (ann->h-PATCH_W)*/) {
              int xpp, ypp;
              if(getnn(ann, x, y+dy, xpp, ypp)) {
              ypp -= dy;
              ...
              if ((xpp != xbest || ypp != ybest) &&
				...
				if(!alreadyCache)
				{
					...
					int *c4row = &((int *) b->line[ypp+yb+PATCH_W-1])[xpp];
					for (int xi = 0; xi < PATCH_W; xi++) {
						aRowLine[0][xi] = c1row[xi];
						bRowLine[0][xi] = c2row[xi];
						aRowLine[1][xi] = c3row[xi];
						bRowLine[1][xi] = c4row[xi];
					}
				}
				else
				{
                ...
                for (int xi = 0; xi < PATCH_W; xi++) {
                  ...
                  //int c3 = c3row[xi];
                  //int c4 = c4row[xi];
                  int dr12 = (aRowLine[0][xi]&255)-(bRowLine[0][xi]&255);
                  int dg12 = ((aRowLine[0][xi]>>8)&255)-((bRowLine[0][xi]>>8)&255);
                  int db12 = ((aRowLine[0][xi]>>16))-((bRowLine[0][xi]>>16));

                  int dr34 = (aRowLine[1][xi]&255)-(bRowLine[1][xi]&255);
                  int dg34 = ((aRowLine[1][xi]>>8)&255)-((bRowLine[1][xi]>>8)&255);
                  int db34 =  ((aRowLine[1][xi]>>16))-((bRowLine[1][xi]>>16));
                  partial +=  dr34*dr34+dg34*dg34+db34*db34
                             -dr12*dr12-dg12*dg12-db12*db12;
                }
                err0 += (dy < 0) ? partial: -partial;
            ...
            if ((unsigned) (x+dx) < (unsigned) (ann->w-PATCH_W)) {
              ...
              if (!IS_WINDOW || window_constraint(p, a, b, x, y, xpp, ypp, ann_window, awinsize)) {
                attempt_n_v2<PATCH_W, IS_MASK, IS_WINDOW>(err, xbest, ybest, adata, b, xpp, ypp, bmask, region_masks, src_mask, p);
              }
            }
...
/* initialize the NN field */
BITMAP *init_nn1(Params *p, BITMAP *a, BITMAP *b, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask, int trim_patch, BITMAP *ann_window, BITMAP *awinsize) {
	BITMAP *bmp = create_bitmap(a->w, a->h);
	clear(bmp);
	//=========================================
	const int maxDx=30;
...
/* TODO: We can use a BITMAP * for region_masks now, no need to calculate the bboxes in RegionMasks constructor. */
/* initialize the NN field */
BITMAP *init_nn_bf(Params *p, BITMAP *a, BITMAP *b, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask, int trim_patch, BITMAP *ann_window, BITMAP *awinsize) {
  BITMAP *bmp = create_bitmap(a->w, a->h);
  clear(bmp);
	int ew = trim_patch ? (b->w - p->patch_w + 1) : b->w;
	int eh = trim_patch ? (b->h - p->patch_w + 1) : b->h;
...
BITMAP *init_dist_v2(Params *p, BITMAP *a, BITMAP *b, BITMAP *ann, BITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
  BITMAP *ans = NULL;

  if (is_window(p)) {
    if      (p->patch_w == 1) { ans = init_dist_n_v2<1,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 2) { ans = init_dist_n_v2<2,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 3) { ans = init_dist_n_v2<3,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 4) { ans = init_dist_n_v2<4,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 5) { ans = init_dist_n_v2<5,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 6) { ans = init_dist_n_v2<6,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 7) { ans = init_dist_n_v2<7,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 8) { ans = init_dist_n_v2<8,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 9) { ans = init_dist_n_v2<9,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 10) { ans = init_dist_n_v2<10,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 11) { ans = init_dist_n_v2<11,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 12) { ans = init_dist_n_v2<12,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 13) { ans = init_dist_n_v2<13,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 14) { ans = init_dist_n_v2<14,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 15) { ans = init_dist_n_v2<15,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 16) { ans = init_dist_n_v2<16,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 17) { ans = init_dist_n_v2<17,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 18) { ans = init_dist_n_v2<18,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 19) { ans = init_dist_n_v2<19,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 20) { ans = init_dist_n_v2<20,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 21) { ans = init_dist_n_v2<21,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 22) { ans = init_dist_n_v2<22,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 23) { ans = init_dist_n_v2<23,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 24) { ans = init_dist_n_v2<24,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 25) { ans = init_dist_n_v2<25,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 26) { ans = init_dist_n_v2<26,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 27) { ans = init_dist_n_v2<27,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 28) { ans = init_dist_n_v2<28,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 29) { ans = init_dist_n_v2<29,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 30) { ans = init_dist_n_v2<30,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 31) { ans = init_dist_n_v2<31,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 32) { ans = init_dist_n_v2<32,1,1>(p, a, b, ann, bmask, region_masks, amask); }
    else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  } 
	else if (amask || bmask || region_masks) {
    //printf("init_dist is masked\n");
    if      (p->patch_w == 1) { ans = init_dist_n_v2<1,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 2) { ans = init_dist_n_v2<2,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 3) { ans = init_dist_n_v2<3,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 4) { ans = init_dist_n_v2<4,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 5) { ans = init_dist_n_v2<5,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 6) { ans = init_dist_n_v2<6,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 7) { ans = init_dist_n_v2<7,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 8) { ans = init_dist_n_v2<8,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 9) { ans = init_dist_n_v2<9,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 10) { ans = init_dist_n_v2<10,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 11) { ans = init_dist_n_v2<11,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 12) { ans = init_dist_n_v2<12,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 13) { ans = init_dist_n_v2<13,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 14) { ans = init_dist_n_v2<14,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 15) { ans = init_dist_n_v2<15,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 16) { ans = init_dist_n_v2<16,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 17) { ans = init_dist_n_v2<17,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 18) { ans = init_dist_n_v2<18,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 19) { ans = init_dist_n_v2<19,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 20) { ans = init_dist_n_v2<20,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 21) { ans = init_dist_n_v2<21,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 22) { ans = init_dist_n_v2<22,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 23) { ans = init_dist_n_v2<23,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 24) { ans = init_dist_n_v2<24,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 25) { ans = init_dist_n_v2<25,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 26) { ans = init_dist_n_v2<26,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 27) { ans = init_dist_n_v2<27,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 28) { ans = init_dist_n_v2<28,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 29) { ans = init_dist_n_v2<29,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 30) { ans = init_dist_n_v2<30,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 31) { ans = init_dist_n_v2<31,1,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 32) { ans = init_dist_n_v2<32,1,0>(p, a, b, ann, bmask, region_masks, amask); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  } 
	else {
    //printf("init_dist is unmasked\n");
    if      (p->patch_w == 1) { ans = init_dist_n_v2<1,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 2) { ans = init_dist_n_v2<2,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 3) { ans = init_dist_n_v2<3,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 4) { ans = init_dist_n_v2<4,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 5) { ans = init_dist_n_v2<5,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 6) { ans = init_dist_n_v2<6,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 7) { ans = init_dist_n_v2<7,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 8) { ans = init_dist_n_v2<8,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 9) { ans = init_dist_n_v2<9,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 10) { ans = init_dist_n_v2<10,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 11) { ans = init_dist_n_v2<11,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 12) { ans = init_dist_n_v2<12,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 13) { ans = init_dist_n_v2<13,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 14) { ans = init_dist_n_v2<14,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 15) { ans = init_dist_n_v2<15,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 16) { ans = init_dist_n_v2<16,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 17) { ans = init_dist_n_v2<17,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 18) { ans = init_dist_n_v2<18,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 19) { ans = init_dist_n_v2<19,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 20) { ans = init_dist_n_v2<20,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 21) { ans = init_dist_n_v2<21,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 22) { ans = init_dist_n_v2<22,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 23) { ans = init_dist_n_v2<23,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 24) { ans = init_dist_n_v2<24,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 25) { ans = init_dist_n_v2<25,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 26) { ans = init_dist_n_v2<26,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 27) { ans = init_dist_n_v2<27,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 28) { ans = init_dist_n_v2<28,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 29) { ans = init_dist_n_v2<29,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 30) { ans = init_dist_n_v2<30,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 31) { ans = init_dist_n_v2<31,0,0>(p, a, b, ann, bmask, region_masks, amask); }
    else if (p->patch_w == 32) { ans = init_dist_n_v2<32,0,0>(p, a, b, ann, bmask, region_masks, amask); }
		else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
  }
  return ans;
}
...
/* Has defaults for params at and after amask. */
void nn_v2(Params *p, BITMAP *a, BITMAP *b,
        BITMAP *ann, BITMAP *annd,
        RegionMasks *amask, BITMAP *bmask,
        int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, int cache_b,
        RegionMasks *region_masks, int tiles, BITMAP *ann_window, BITMAP *awinsize) {
  ...
  if (algo == ALGO_CPU) {
    if (is_window(p)) {
      printf("Running nn, using windowed and masked\n");
      if      (p->patch_w == 1) { nn_n_v2<1,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 2) { nn_n_v2<2,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 3) { nn_n_v2<3,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 4) { nn_n_v2<4,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 5) { nn_n_v2<5,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 6) { nn_n_v2<6,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 7) { nn_n_v2<7,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 8) { nn_n_v2<8,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 9) { nn_n_v2<9,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 10) { nn_n_v2<10,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 11) { nn_n_v2<11,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 12) { nn_n_v2<12,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 13) { nn_n_v2<13,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 14) { nn_n_v2<14,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 15) { nn_n_v2<15,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 16) { nn_n_v2<16,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
			else if (p->patch_w == 17) { nn_n_v2<17,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 18) { nn_n_v2<18,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 19) { nn_n_v2<19,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 20) { nn_n_v2<20,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 21) { nn_n_v2<21,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 22) { nn_n_v2<22,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 23) { nn_n_v2<23,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 24) { nn_n_v2<24,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 25) { nn_n_v2<25,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 26) { nn_n_v2<26,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 27) { nn_n_v2<27,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 28) { nn_n_v2<28,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 29) { nn_n_v2<29,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 30) { nn_n_v2<30,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 31) { nn_n_v2<31,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 32) { nn_n_v2<32,1,1>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else if (bmask == NULL && amask == NULL && region_masks == NULL) {
      //printf("Running nn, using unmasked\n");
      if      (p->patch_w == 1) { nn_n_v2<1,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 2) { nn_n_v2<2,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 3) { nn_n_v2<3,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 4) { nn_n_v2<4,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 5) { nn_n_v2<5,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 6) { nn_n_v2<6,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 7) { nn_n_v2<7,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 8) { nn_n_v2<8,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 9) { nn_n_v2<9,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 10) { nn_n_v2<10,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 11) { nn_n_v2<11,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 12) { nn_n_v2<12,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 13) { nn_n_v2<13,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 14) { nn_n_v2<14,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 15) { nn_n_v2<15,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 16) { nn_n_v2<16,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 17) { nn_n_v2<17,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 18) { nn_n_v2<18,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 19) { nn_n_v2<19,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 20) { nn_n_v2<20,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 21) { nn_n_v2<21,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 22) { nn_n_v2<22,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 23) { nn_n_v2<23,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 24) { nn_n_v2<24,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 25) { nn_n_v2<25,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 26) { nn_n_v2<26,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 27) { nn_n_v2<27,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 28) { nn_n_v2<28,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 29) { nn_n_v2<29,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 30) { nn_n_v2<30,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 31) { nn_n_v2<31,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 32) { nn_n_v2<32,0,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    } 
		else {
      printf("Running nn, using masked\n");
      if      (p->patch_w == 1) { nn_n_v2<1,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 2) { nn_n_v2<2,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 3) { nn_n_v2<3,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 4) { nn_n_v2<4,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 5) { nn_n_v2<5,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 6) { nn_n_v2<6,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 7) { nn_n_v2<7,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 8) { nn_n_v2<8,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 9) { nn_n_v2<9,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 10) { nn_n_v2<10,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 11) { nn_n_v2<11,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 12) { nn_n_v2<12,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 13) { nn_n_v2<13,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 14) { nn_n_v2<14,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 15) { nn_n_v2<15,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 16) { nn_n_v2<16,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 17) { nn_n_v2<17,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 18) { nn_n_v2<18,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 19) { nn_n_v2<19,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 20) { nn_n_v2<20,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 21) { nn_n_v2<21,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 22) { nn_n_v2<22,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 23) { nn_n_v2<23,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 24) { nn_n_v2<24,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 25) { nn_n_v2<25,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 26) { nn_n_v2<26,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 27) { nn_n_v2<27,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 28) { nn_n_v2<28,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 29) { nn_n_v2<29,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 30) { nn_n_v2<30,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 31) { nn_n_v2<31,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else if (p->patch_w == 32) { nn_n_v2<32,1,0>(p, a, b, ann, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, region_masks, tiles, ann_window, awinsize); }
      else { fprintf(stderr, "Patch size unsupported: %d\n", p->patch_w); exit(1); }
    }
  } 
	else {
...
inline bool getnn(BITMAP *ann, int x, int y, int &xp, int &yp) {
